...
import framework.core.classes.TestObjectsHandler;
...
public class ExecuteTestObjectsParallely {
	...
	public static TreeMap<Integer, TestCaseObjects> testCaseObjectMap;
	...
	public void beforeClass() 
	{
		try
		{
			...
			connectionServe = GenericMethodsLib.CreateSQLConnection();
			...
			testCaseObjectMap = new TestObjectsHandler().getTestCaseObjectMap(readTest.getRunnableTestCaseObjects(testCaseFile));

			/** set the completable future array size */
			completableFutureObjects = new TestObjectsHandler().getCompletableFuture(testCaseObjectMap);
		}
		catch (Exception e)
		{
	...
	public void runTests()
	{
		...
		while(!new TestObjectsHandler().ifAllTestCaseObjectsAreExecuted())
		{
			/** handle code */
			try {
				Thread.sleep(5000);
	...
	public Object assignTasks()
	{
		for(CompletableFuture<?> futureObject : completableFutureObjects)
		{
			...
			TreeMap<Integer, TestCaseObjects> testCaseObjectMapToBeExecuted = new TestObjectsHandler().getFreeTestCase();
			...
			{	
				...
				CompletableFuture<?> futureObjectOutput = futureObject.supplyAsync(() -> new TestObjectsHandler().
